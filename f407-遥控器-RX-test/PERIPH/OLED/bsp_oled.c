...
void OLED_Init(void)
{
	...
	I2C_WriteCmd(0x20);	//Set Memory Addressing Mode	
	I2C_WriteCmd(0x10);	//00,Horizontal Addressing Mode;01,Vertical Addressing Mode;10,Page Addressing Mode (RESET);11,Invalid
	I2C_WriteCmd(0xb0);	//Set Page Start Address for Page Addressing Mode,0-7
	I2C_WriteCmd(0xc8);	//Set COM Output Scan Direction
	I2C_WriteCmd(0x00); //---set low column address
	I2C_WriteCmd(0x10); //---set high column address
	I2C_WriteCmd(0x40); //--set start line address
	I2C_WriteCmd(0x81); //--set contrast control register
	I2C_WriteCmd(0xff); //亮度调节 0x00~0xff
	I2C_WriteCmd(0xa1); //--set segment re-map 0 to 127
	I2C_WriteCmd(0xa6); //--set normal display
	I2C_WriteCmd(0xa8); //--set multiplex ratio(1 to 64)
	I2C_WriteCmd(0x3F); //
	I2C_WriteCmd(0xa4); //0xa4,Output follows RAM content;0xa5,Output ignores RAM content
	I2C_WriteCmd(0xd3); //-set display offset
	I2C_WriteCmd(0x00); //-not offset
	I2C_WriteCmd(0xd5); //--set display clock divide ratio/oscillator frequency
	I2C_WriteCmd(0xf0); //--set divide ratio
	I2C_WriteCmd(0xd9); //--set pre-charge period
	I2C_WriteCmd(0x22); //
	I2C_WriteCmd(0xda); //--set com pins hardware configuration
	I2C_WriteCmd(0x12);
	I2C_WriteCmd(0xdb); //--set vcomh
	I2C_WriteCmd(0x20); //0x20,0.77xVcc
	I2C_WriteCmd(0x8d); //--set DC-DC enable
	I2C_WriteCmd(0x14); //
	I2C_WriteCmd(0xaf); //--turn on oled panel
}
...
void OLED_Refresh_Gram(void)
{
  ...
  I2C_WriteCmd(0x21);
  I2C_WriteCmd(0);   // Column start address (0 = reset)
  I2C_WriteCmd(OLED_WIDTH-1); // Column end address (127 = reset)

  I2C_WriteCmd(0x22);
  I2C_WriteCmd(0); // Page start address (0 = reset)
  I2C_WriteCmd(7); // Page end address

  for(int i=0; i<OLED_WIDTH*OLED_HEIGHT/8; i++){
    I2C_WriteDat(OLED_GRAM[i]);
  } 
}
//OLED整屏填充
...
{
	I2C_WriteCmd(0X8D);  //SET DCDC命令
	I2C_WriteCmd(0X14);  //DCDC ON
