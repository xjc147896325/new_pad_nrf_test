...
const char *g_CheckOKString = "RF24L01 has    find !...\r\n";
...
// * @retval:无
// */
//oid NRF24L01_Set_Speed( nRf24l01SpeedType Speed )
//
//	uint8_t btmp = 0;
//	
...
void NRF24L01_check( void )
{
	...
	while( 1 )
	{
		...
		if( 5 == i )
		{
			drv_uart_tx_bytes( (uint8_t *)g_CheckOKString, 28 );
			break;
		}
		else
		{
			drv_uart_tx_bytes( (uint8_t *)g_ErrorString, 28 );
		}
		Delay_TIM3_ms( 2000 );
	}
}
...
{
	uint8_t l_Status = 0, l_RxLength = 0, l_100MsTimes = 0, f_Status = 0, Flag_IRQ = 1;
	
	RF24L01_SET_CS_LOW( );		//片选
	drv_spi_read_write_byte( FLUSH_RX );
	l_Status = NRF24L01_Read_Reg(STATUS);						//读状态寄存器
	f_Status = NRF24L01_Read_Reg(0x17);						//读状态寄存器
	Delay_TIM3_ms( 1 );
	RF24L01_SET_CS_HIGH( );
	
	while( 0 != Flag_IRQ)
	{
		Flag_IRQ = RF24L01_GET_IRQ_STATUS( );
		Delay_TIM3_ms( 100 );
...
/*------------------test-------------------*/
    NRF24L01_Write_Reg( EN_AA, 0x00 );   					//通道0自动应答失能
//    NRF24L01_Write_Reg( EN_RXADDR, 0x00 );					//通道0接收失能
